 ...
 *
 * Inicia el OAuth de Instagram. Se dispara con una navegación full-page desde
...
export async function GET(request: NextRequest) {
  ...
  if (!isInstagramConfigured()) {
    return Response.redirect(`${clientesUrl}?ig_error=not_configured`, 302)
  }
  ...
  if (!clientId) {
    return Response.redirect(`${clientesUrl}?ig_error=token`, 302)
  }
  ...
  res.headers.append(
    'Set-Cookie',
    `ig_oauth_state=${nonce}; Path=/; HttpOnly; SameSite=Lax; Max-Age=600${
